...
- **Constants that are truly fixed** (thresholds, timing values, the bid-increment schedule's boundaries): `UPPER_SNAKE_CASE`, no magic numbers inline in logic — a bare `3` or `24` in the middle of a function is unreviewable without a name attached. One genuinely shared across otherwise-unrelated files (`WATCHLIST_HIGHLIGHT_MS`, `URGENT_THRESHOLD_HOURS`) belongs in `utils/constants.ts`, imported everywhere it's used; one that's a private implementation detail of a single file (`ws.ts`'s own reconnect-backoff timing, `lifecycle.ts`'s derived `AUCTION_DURATION_MS`) stays a local top-level `const` in that file instead — centralizing something nothing else references just adds an import for no shared benefit.
