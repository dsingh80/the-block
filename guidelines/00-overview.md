...
| Repo layout | Monorepo: `client/` (this app, a fully self-contained npm project) + `server/` (a real Go module — the Redis/Postgres backend, `06-backend-architecture.md`) + `guidelines/` at the repo root, governing both. | Matches the handoff doc's intended shape. |
...
| Auction lifecycle clock | `upcoming`/`active`/`ended` is derived from real `Date.now()`, both client-side (`utils/lifecycle.ts`) and server-side (`internal/domain/lifecycle.go`) — never a synthetic/normalized "now". | **Deviation on record**: an earlier draft of the implementation plan computed a synthetic reference time from the dataset's own timestamp range, specifically to guarantee a live mix of upcoming/active/ended listings during a demo. Rejected in review in favor of real time. No longer the staleness tradeoff it once was, now that a real backend exists: the generator (`scripts/generate_vehicles.mjs`) sets every `auction_start` 1–7 days after whenever it's actually run, and `cmd/reconcile` re-syncs `data/vehicles.json` into Postgres/Redis on every `docker compose up` (`06-backend-architecture.md`, "Data lifecycle") — so regenerating the dataset before starting the stack always produces a live upcoming/active mix, no code change or workaround needed. |
